...
Vincent 用来写周记、笔记、随手记的个人网站。基于 [Astro](https://astro.build/) 静态博客，**不需要数据库、不需要服务器**，写作就是在电脑上编辑文本文件，别人可以随时点进来阅读（没有评论、登录等交互）。

---

## 一、这个网站长什么样

- **首页**：把所有记录按时间从新到旧排好（卡片式），每条带一个分类标签（周记 / 笔记 / 随手记）。
- **全部记录页**：可按分类查看，顶部有各分类的数量统计。
- **学习报告页**：放自己做的网页版学习报告，把 `.html` 拷进去就自动出现。
- **文章页**：单条记录的完整内容。
- **关于页**：自我介绍，可自由修改。
...
| `category` | 否 | 分类，只能是 `weekly`(周记) / `notes`(笔记) / `quick`(随手记)，不写默认随手记 |
...
学习报告是**独立的网页文件**，不走 Markdown。按「**文件夹 = 分类**」组织：

1. 在 `public/reports/` 下建一个分类文件夹（比如「个人成长」）。
2. 把生成好的、自带样式的 `.html` 文件拷进这个文件夹。
3. `git push`（或本地 `npm run dev` 预览）后，它会**自动出现**在「学习报告」页，并归到对应分类下。
4. 列表里显示的标题，取自 html 里的 `<title>...</title>`；没写就用文件名。

> 例如把 `成长的底层逻辑.html` 放进 `public/reports/个人成长/`，它会显示在「个人成长」分类里，
> 访问地址是 `你的网址/reports/个人成长/成长的底层逻辑.html`。
> 想再开一个分类，就再建一个文件夹即可。直接丢在 `public/reports/` 根目录（不放进任何文件夹）的 html 会归到「其他」。
...
| 分类（增删「周记/笔记/随手记」） | `src/consts.ts` 里的 `CATEGORIES`，同时改 `src/content.config.ts` 里的 `category` 枚举 |
| 「关于」页内容 | `src/pages/about.astro` |
| 导航栏文字 | `src/components/Header.astro` |
| 整体配色（奶蓝色）、字体大小 | `src/styles/global.css` |
