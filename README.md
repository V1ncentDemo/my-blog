...
学习报告是**独立的网页文件**，不走 Markdown。步骤很简单：

1. 把你生成好的、自带样式的 `.html` 文件，直接拷进 `public/reports/` 文件夹。
2. `git push`（或本地 `npm run dev` 预览）后，它会**自动出现**在「学习报告」列表里。
3. 列表里显示的标题，取自 html 里的 `<title>...</title>`；没写就用文件名。

> 例如把 `2026-08-w3-学习报告.html` 放进 `public/reports/`，访问地址就是
> `你的网址/reports/2026-08-w3-学习报告.html`。文件名尽量用英文/数字/短横线更稳妥。
